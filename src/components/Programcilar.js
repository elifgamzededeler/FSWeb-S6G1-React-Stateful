...
export default function Programcilar() {
  ...
  const [oneCikan, setOneCikan] = useState(null);
  ...
  const oneCikaninIsmi = () => {
    return programcilar[oneCikan - 1].isim;
    // Bunu sona bırakın!
  ...
  const stil = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: oneCikan == null ? "royalblue" : "gold", // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className="programmers">
        {
          ...
          enIyilerListesi.map((dev) => (
            <div className="programmer" key={dev.id}>
              {dev.isim} {/*butonun içinde bunu yazıyoruz yani dev.isim}*/}
              <button
                onClick={() => {
                  setOneCikan(dev.id);
                  /* burada dev.id 'yi öne çıkan id'ye atayın */ //butona onclick yaptığında öneçıkanı dev.id olarak set et.
                }}
              >
                Kutla{" "}
                {/*butonda kutla yazsın (yani kutla yazan butona tıkladığında yukarıdaki set yazdığımız şeyi set etsin*/}
              </button>
            </div>
          ))
        }
